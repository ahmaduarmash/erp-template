...
function isGroupAccount(row: RecordData) {
  return Number(String(row.code).slice(-1)) % 2 === 0;
}

function allowReconciliation(row: RecordData) {
  return Number(String(row.code).slice(-1)) % 3 === 0;
}
...
export default function ChartOfAccountsPage() {
  ...
  const balance = selected
    ? Number(String(selected.code).replace(/\D/g, '')) * 13
    : 0;
  ...
  return (
    <MotionSurface page>
      ...
      <div className="coa-workspace">
        <aside className="panel coa-browser">
          <div className="coa-browser-head">
            <div>
              <span className="coa-eyebrow">Account structure</span>
              <h2>Ledger hierarchy</h2>
            </div>
            <Segmented
              size="small"
              value={view}
              onChange={(next) => setView(next as 'tree' | 'list')}
              options={[
        ...
        <section className="panel coa-detail">
          {selected ? (
            <>
                ...
                <div className="coa-account-identity">
                  ...
                  <div>
                    <div className="coa-account-meta">
                      ...
                      <span>•</span>
                      <span>{isGroupAccount(selected) ? 'Group account' : 'Posting account'}</span>
                    </div>
                    <h2>{selected.name}</h2>
              ...
              <div className="coa-command-bar">
                <Tooltip title="View ledger">
                  <Button icon={<FileSearchOutlined />} onClick={() => feedback.success(`Opening ledger for ${selected.name}`)} />
                </Tooltip>
                <Tooltip title="Add child account">
                  <Button icon={<PlusOutlined />} onClick={() => {
                    form.setFieldsValue({ parent: selected.name, type: selected.type, active: true });
                    setOpen(true);
                  }} />
                </Tooltip>
                <Tooltip title="Edit account">
                  <Button icon={<EditOutlined />} onClick={() => feedback.success('Edit account flow ready')} />
                </Tooltip>
                <Tooltip title="Deactivate">
                  <Button
                    ...
                    onClick={() => workspace.save('chart-of-accounts', { ...selected, status: 'Inactive' })}
                  />
                </Tooltip>
              </div>

              <div className="coa-balance-card">
                <div>
                  <span>Current balance</span>
                  <strong>{formatMoney(balance, workspace.org.currency)}</strong>
                  <small>Demo balance from the workspace adapter</small>
                </div>
                <div className="coa-balance-icon"><WalletOutlined /></div>
              </div>

              <div className="coa-detail-grid">
                <div className="coa-info-card">
                  <span>Posting mode</span>
                  <strong>{isGroupAccount(selected) ? 'Group account' : 'Posting account'}</strong>
                  <small>{isGroupAccount(selected) ? 'Organizes child accounts for reporting.' : 'Accepts journal postings.'}</small>
                </div>
                <div className="coa-info-card">
                  <span>Reconciliation</span>
                  <strong>{allowReconciliation(selected) ? 'Allowed' : 'Not enabled'}</strong>
                  <small>{allowReconciliation(selected) ? 'Suitable for open-item matching.' : 'Standard ledger behavior.'}</small>
                </div>
                <div className="coa-info-card">
              ...
              <div className="coa-section">
                <div className="coa-section-head">
                  <div>
                    <span className="coa-eyebrow">Configuration</span>
            ...
            <div className="coa-empty">
              <FolderOpenOutlined />
              <Typography.Title level={4}>Select an account</Typography.Title>
              <Typography.Text type="secondary">Choose an account from the hierarchy to inspect its configuration and ledger context.</Typography.Text>
            </div>
          )}
        </section>
      </div>
      ...
      <Drawer
        className="erp-form-drawer"
        width={620}
        open={open}
        title="Create account"
        onClose={() => setOpen(false)}
        extra={<Tag color="processing">Accounting</Tag>}
        footer={
          <Space>
            <Button onClick={() => setOpen(false)}>Cancel</Button>
            <Button type="primary" onClick={() => void createAccount()}>Create account</Button>
          </Space>
        }
      >
        <div className="form-intro">
          <div className="form-intro-icon"><BookOutlined /></div>
          <div>
            <h3>Account details</h3>
            <p>Keep account codes predictable and use group accounts only for hierarchy—not for postings.</p>
          </div>
        </div>
        <Form form={form} layout="vertical" initialValues={{ type: 'Asset', active: true, isGroup: false }}>
          <div className="form-grid">
            <Form.Item name="name" label="Account name" rules={[{ required: true, message: 'Account name is required' }]}>
              <Input placeholder="e.g. Main bank account" />
            </Form.Item>
            <Form.Item name="code" label="Account code" rules={[{ required: true, message: 'Account code is required' }]}>
              <Input placeholder="e.g. 1110" />
            </Form.Item>
            <Form.Item name="type" label="Account type" rules={[{ required: true }]}>
              <Select options={accountTypes.map((value) => ({ value, label: value }))} />
            </Form.Item>
            <Form.Item name="parent" label="Parent account">
              <Select
                allowClear
                showSearch
                placeholder="Optional"
                options={rows.map((row) => ({ value: row.name, label: `${row.code} · ${row.name}` }))}
              />
            </Form.Item>
          </div>
          <div className="form-switch-list">
            <div className="form-switch-row">
              <div><strong>Group account</strong><span>Use only to organize child accounts.</span></div>
              <Form.Item name="isGroup" valuePropName="checked" noStyle><Switch /></Form.Item>
            </div>
            <div className="form-switch-row">
              <div><strong>Allow reconciliation</strong><span>Enable open-item matching for this ledger.</span></div>
              <Form.Item name="reconcile" valuePropName="checked" noStyle><Switch /></Form.Item>
            </div>
            <div className="form-switch-row">
              <div><strong>Active</strong><span>Available for selection in financial documents.</span></div>
              <Form.Item name="active" valuePropName="checked" noStyle><Switch /></Form.Item>
            </div>
          </div>
        </Form>
      </Drawer>
